...
def main():
    ...
    # Next Generate stego-images, r=1.0
    stegolist = selector.select(TrainSize)
    path = "../data/final/" + str(RUN) + "/lists/tstego.txt"
    checkpath(path)
    file = open(path, "w")
    file.write(",".join(stegolist))
    file.close()
    for im in stegolist:
        imstat.writeStats(im, "imstats/final/" + str(RUN) + "/tstego/", img=randLsb.rand(im, 1.0, flatten=True), methods=methods)
    print "Done Stego Training"
